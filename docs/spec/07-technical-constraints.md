...
- **The set tail** is the next three practices in the editorial order not already on screen, scanning onward from the set's last practice and wrapping.
  - **Until the catalog is larger than a set plus three, the tail is short rather than padded.** "Not already on screen" and "the same size on every screen" cannot both hold while fewer than seven practices are published: a build with three would have to repeat a practice the reader is already looking at to fill the tail. The tail holds what is left instead, and the constant size — the part that stops a screen revealing how well-stocked an answer was — starts holding on its own once the twelve slots of the editorial order are written. Nothing in the code caps or pads a tail to hide the gap ([#25](https://github.com/inarush0/spiritual-collective/issues/25)).
