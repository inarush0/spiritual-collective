...
- **The editor is a single point of failure for execution, unavoidably.** This asymmetry is over authority, not capability: the chaplain reviewer can force the removal of anything shipped, and the editor is who performs it. The chaplain reviewer holds no registrar, DNS, host, or repository credential and is not asked to — they are an unpaid volunteer with no technical role, and a second executor is not something two people can produce. So during an editor absence a harm report is read, triaged, and answered, and **nothing can be taken down**. The resource states that on the page for as long as it is true rather than leaving a reader to wait on it ([`docs/runbooks/accountability-mailbox.md`](../runbooks/accountability-mailbox.md)). Unlike the publishing bottleneck above, this one is not chosen — it is the standing capacity, written down so that no procedure quietly assumes otherwise.
...
Decided in [#9](https://github.com/inarush0/spiritual-collective/issues/9), amended by [#13](https://github.com/inarush0/spiritual-collective/issues/13) (signatures carry role, not identity), [#14](https://github.com/inarush0/spiritual-collective/issues/14) (a voided approval never unpublishes; the guide record is a second governed kind), and [#39](https://github.com/inarush0/spiritual-collective/issues/39) (the asymmetry is over authority, not capability; the editor is the single executor). Operational detail in [`docs/spec/05-governance.md`](../spec/05-governance.md).
